...
def equal_weight_allocation(
    ...
    """
    Step 3: Assign equal weights to QAOA-selected assets.

    QAOA already performs binary combinatorial optimization to select
    the best asset subset — no further classical weight tuning is needed.
    Each selected asset receives weight 1/N.
    """
    n = len(selected_assets)
    if n == 0:
    ...
    allocation = {ticker: weight for ticker in selected_assets}

    # Calculate portfolio metrics using equal weights
    price_df = pd.DataFrame({t: prices_dict[t] for t in selected_assets})
    returns = price_df.pct_change().dropna()
...
# ============================================================================
    ...
    if not hasattr(st.session_state, 'allocation'):
        st.markdown("## ⚖️ Equal-Weight Allocation")
        st.info("QAOA already solved the binary asset-selection problem. Each selected asset receives an equal weight (1/N).")

        with st.spinner("Computing equal-weight allocation..."):
            allocation, metrics = equal_weight_allocation(
        ...
        st.markdown("### 💼 Portfolio Allocation (Equal Weight)")
