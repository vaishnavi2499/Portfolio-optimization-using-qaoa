...
def build_qaoa_problem(
    ...
    if not QISKIT_AVAILABLE:
        raise ImportError(
            "Qiskit not available. Install with: pip install qiskit qiskit-optimization qiskit-algorithms")
...
def _curve_stats(returns_series: pd.Series, initial_capital: float, rf_rate: float):
    """Compute performance stats from a daily returns series."""
    curve = initial_capital * (1 + returns_series).cumprod()
    final_value = curve.iloc[-1]
    total_return = (final_value / initial_capital) - 1
    years = len(returns_series) / 252
    cagr = (1 + total_return) ** (1 / years) - 1 if years > 0 else 0
    vol = returns_series.std() * np.sqrt(252)
    sharpe = (returns_series.mean() * 252 - rf_rate) / vol if vol > 0 else 0
...
def run_backtest(
    ...
    """
    Run a buy-and-hold backtest on the test split (last 30% of data).

    Transaction costs
    -----------------
    Applied as a one-time entry drag at the start of the test period.
    Each position pays `transaction_cost_bps` bps on entry (buy) and
    `transaction_cost_bps` bps on exit (sell at end), so total round-trip
    cost = 2 × cost_per_leg × weight per asset, summed across all assets.

    Index benchmark
    ---------------
    Attempts to load the index (Nifty 500 or S&P 500) via fetch_benchmark_api
    for the same test-period date range. Falls back silently if unavailable.
    """
    tickers = list(allocation.keys())
    weights = np.array([allocation[t] for t in tickers])

    # Align price data across portfolio assets
    price_df = pd.concat(
    ...
    # Round-trip cost = 2 legs × cost_per_leg; applied as a one-time
    # capital reduction at the start of the test period.
    cost_per_leg = transaction_cost_bps / 10_000  # e.g. 20 bps → 0.002
    round_trip_cost = 2 * cost_per_leg  # entry + exit leg
    # Total drag = sum of (weight_i × round_trip_cost) for all i
    # Since weights sum to 1 this simplifies to round_trip_cost itself,
    # but we keep the explicit form so it's correct if weights are later
    # constrained to a subset.
    total_cost_fraction = np.dot(weights, np.ones(len(weights))) * round_trip_cost
    capital_after_costs = initial_capital * (1 - total_cost_fraction)

    # ── Portfolio curve (optimized weights, after costs) ──────────────────
    port_daily = test_returns.dot(weights)
    port_stats = _curve_stats(port_daily, capital_after_costs, rf_rate)
    # Re-express final value relative to the original capital so
    # total_return is meaningful vs. gross investment
    port_stats['total_return'] = (port_stats['final_value'] / initial_capital) - 1
    port_stats['cost_drag'] = total_cost_fraction  # expose for display

    # ── Equal-weight benchmark (same assets, no cost adjustment) ──────────
    eq_weights = np.ones(len(tickers)) / len(tickers)
    eq_daily = test_returns.dot(eq_weights)
    eq_stats = _curve_stats(eq_daily, initial_capital, rf_rate)

    # ── Index benchmark (Nifty 500 or S&P 500) ────────────────────────────
    index_stats = None
    index_label = 'Nifty 500' if 'nifty' in universe.lower() else 'S&P 500'
    index_ticker = '^NSEI' if 'nifty' in universe.lower() else '^GSPC'

    if LOADERS_AVAILABLE:
        try:
            idx_df = fetch_benchmark_api(index_ticker)

            # Normalise index date column
            if 'date' in idx_df.columns:
                idx_df['date'] = pd.to_datetime(idx_df['date'], errors='coerce')
                if idx_df['date'].dt.tz is not None:
                    idx_df['date'] = idx_df['date'].dt.tz_localize(None)
            ...
            for col in ('close', 'adj_close', 'Close', 'Adj Close'):
                if col in idx_df.columns:
                    idx_prices = idx_df[col]
                    break
            else:
                idx_prices = idx_df.iloc[:, 0]

            # Align to test period
            idx_prices = idx_prices.loc[
                (idx_prices.index >= test_start) &
                (idx_prices.index <= test_end)
                ]

            if len(idx_prices) >= 20:
                idx_returns = idx_prices.pct_change().dropna()
                # Reindex to match portfolio dates (inner join)
                idx_returns = idx_returns.reindex(test_returns.index).dropna()
                if len(idx_returns) >= 20:
                    index_stats = _curve_stats(idx_returns, initial_capital, rf_rate)
                    index_stats['label'] = index_label

        except Exception as e:
            print(f"Index benchmark load failed ({index_ticker}): {e}")

    return {
        # Portfolio
        'portfolio_curve': port_stats['curve'],
        'final_value': port_stats['final_value'],
        'total_return': port_stats['total_return'],
        'cagr': port_stats['cagr'],
        'volatility': port_stats['volatility'],
        'sharpe': port_stats['sharpe'],
        'max_drawdown': port_stats['max_drawdown'],
        'cost_drag': port_stats['cost_drag'],
        # Equal-weight benchmark
        'benchmark_curve': eq_stats['curve'],
        'bench_final': eq_stats['final_value'],
        'bench_return': eq_stats['total_return'],
        'bench_cagr': eq_stats['cagr'],
        'bench_vol': eq_stats['volatility'],
        'bench_sharpe': eq_stats['sharpe'],
        'bench_drawdown': eq_stats['max_drawdown'],
        # Index benchmark (None if unavailable)
    ...
    with st.form("config_form"):
        ...
        with col2:
            ...
            transaction_cost_bps = st.slider(
                ...
                step=5,
                help=(
                    "One-way cost per trade in basis points (1 bps = 0.01%). "
                    "Includes brokerage + STT + impact. "
                    "Typical: 10–20 bps for large-cap, 30–50 bps for mid-cap."
                )
            )
...
# ============================================================================
        ...
        if 'top_portfolios' in st.session_state.results_dict and st.session_state.results_dict['top_portfolios']:
            ...
            for rank, portfolio in enumerate(top_portfolios, 1):
                ...
                portfolio_data.append({
                    'Rank': rank,
                    'Probability': f"{portfolio.get('probability', 0) * 100:.2f}%" if portfolio.get(
                        'probability') else 'N/A',
                    'Expected Return': f"{portfolio.get('expected_return', 0) * 100:.2f}%" if portfolio.get(
                        'expected_return') is not None else 'N/A',
                    'Risk (Vol)': f"{portfolio.get('risk', 0) * 100:.2f}%" if portfolio.get(
                        'risk') is not None else 'N/A',
                    'Sharpe Ratio': f"{portfolio.get('sharpe', 0):.2f}" if portfolio.get(
                        'sharpe') is not None else 'N/A',
            ...
            if selected_rank > 0:
                ...
                st.info(
                    f"Selected alternative portfolio (Rank {selected_rank}). Proceeding to equal-weight allocation.")
    ...
    if not hasattr(st.session_state, 'allocation'):
        ...
        backtest_results = run_backtest(
            ...
            rf_rate=st.session_state.rf_rate,
            transaction_cost_bps=st.session_state.get('transaction_cost_bps', 20),
        ...
        if backtest_results is None:
            ...
            # ── Transaction cost callout ──────────────────────────────────
            cost_drag_pct = backtest_results['cost_drag'] * 100
            cost_drag_inr = backtest_results['cost_drag'] * st.session_state.investment
            st.info(
                f"💸 **Transaction cost drag:** {cost_drag_pct:.3f}% "
                f"(₹{cost_drag_inr:,.0f} on ₹{st.session_state.investment:,.0f}) — "
                f"round-trip ({st.session_state.get('transaction_cost_bps', 20)} bps entry + "
                f"{st.session_state.get('transaction_cost_bps', 20)} bps exit). "
                f"Portfolio return shown is **after** this cost."
            )

            # ── Headline metrics: Portfolio vs Equal-weight ───────────────
            col1, col2, col3, col4 = st.columns(4)
            ...
            # ── Performance chart: 3 lines ────────────────────────────────
            fig = go.Figure()

            # 1. Optimized portfolio (after costs)
            fig.add_trace(go.Scatter(
                x=backtest_results['portfolio_curve'].index,
                y=backtest_results['portfolio_curve'].values,
                mode='lines',
                name='Optimized Portfolio (after costs)',
            ...
            # 3. Index benchmark (if loaded)
            index_stats = backtest_results.get('index_stats')
            if index_stats is not None:
                fig.add_trace(go.Scatter(
                    x=index_stats['curve'].index,
                    y=index_stats['curve'].values,
                    mode='lines',
                    name=backtest_results['index_label'],
                    line=dict(color='#a78bfa', width=1.5, dash='dot')
                ))
            else:
                st.caption(
                    f"ℹ️ {backtest_results['index_label']} index data unavailable "
                    f"(fetch_benchmark_api returned no data). "
                    f"Showing equal-weight baseline only."
                )

            fig.update_layout(
                title=f"Portfolio vs Benchmarks — test period only (after {st.session_state.get('transaction_cost_bps', 20)} bps costs)",
                paper_bgcolor='rgba(0,0,0,0)',
                plot_bgcolor='rgba(15,12,29,0.8)',
                font=dict(color='#9ca3af'),
            ...
            with col3:
                st.metric("Eq-Wt CAGR", f"{backtest_results['bench_cagr']:.2%}")

            with col4:
                st.metric("Eq-Wt Sharpe", f"{backtest_results['bench_sharpe']:.2f}")

            # ── Index comparison row (only if index loaded) ───────────────
            if index_stats is not None:
                st.markdown(f"### 📊 vs {backtest_results['index_label']}")
                col1, col2, col3, col4 = st.columns(4)
                with col1:
                    st.metric(
                        f"{backtest_results['index_label']} Final",
                        f"₹{index_stats['final_value']:,.0f}"
                    )
                with col2:
                    st.metric(
                        f"{backtest_results['index_label']} CAGR",
                        f"{index_stats['cagr']:.2%}",
                    )
                with col3:
                    st.metric(
                        f"{backtest_results['index_label']} Sharpe",
                        f"{index_stats['sharpe']:.2f}"
                    )
                with col4:
                    st.metric(
                        "Portfolio α (CAGR)",
                        f"{backtest_results['cagr'] - index_stats['cagr']:.2%}",
                        delta="vs index"
                    )