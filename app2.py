...
2. Real Quantum QAOA → Discrete combinatorial optimization
3. Classical post-optimization → Mean-variance weight optimization
...
def build_qaoa_problem(
    ...
    # Constraint: select exactly n_select assets
    linear_constraint = {f'x_{i}': 1 for i in range(n)}
    # -------------------------
...
def solve_qaoa(
    ...
    return selected_assets, "Classical (Greedy)"
...
def classical_weight_optimization(
        selected_assets: List[str],
        prices_dict: Dict[str, pd.Series],
        max_position: float,
        objective: str,
    ...
    """
    Step 3: Optimize portfolio weights for selected assets

    Classical mean-variance optimization on quantum-selected assets
    """
    # Prepare data
    price_df = pd.DataFrame({t: prices_dict[t] for t in selected_assets})
    returns = price_df.pct_change().dropna()
    ...
    if objective == 'max_sharpe':
        def neg_sharpe(weights):
            port_return = np.dot(weights, expected_returns)
            port_vol = np.sqrt(np.dot(weights, np.dot(cov_matrix, weights)))
            return -(port_return - rf_rate) / port_vol if port_vol > 0 else 1e10
    ...
    bounds = tuple((0, max_position) for _ in range(n))
    ...
    allocation = {
        ticker: weight
        for ticker, weight in zip(selected_assets, result.x)
        if weight > 0.001
    }
    ...
    port_return = np.dot(weights, expected_returns[tickers])
    port_vol = np.sqrt(np.dot(weights, np.dot(cov_matrix.loc[tickers, tickers], weights)))
    sharpe = (port_return - rf_rate) / port_vol if port_vol > 0 else 0
...
def run_backtest(allocation, prices_dict, initial_capital):
    tickers = list(allocation.keys())
    ...
    sharpe = (
        (portfolio_returns.mean() * 252 - 0.04) /
    ...
    with st.form("config_form"):
        ...
        with col2:
            ...
            risk_aversion = st.slider(
                ...
                value=0.5,
                step=0.1,
                help="Higher = more conservative portfolio"
            )
    ...
    if not hasattr(st.session_state, 'selected_assets'):
        ...
        with st.spinner(f"Running QAOA (depth={st.session_state.qaoa_depth})..."):
            ...
            results_dict, solver_type = solve_qaoa(
                ...
                prices_dict=st.session_state.prices_dict,  # NEW
                rf_rate=0.04,  # NEW
        ...
        if 'top_portfolios' in st.session_state.results_dict and st.session_state.results_dict['top_portfolios']:
            ...
            selected_rank = st.selectbox(
                "Select alternative portfolio to use for weight optimization:",
            ...
            if selected_rank > 0:
                ...
                st.info(f"Selected alternative portfolio (Rank {selected_rank}). Proceed to weight optimization.")

                st.session_state.step = 2
                st.rerun()

    # Step 3: Classical Weight Optimization
    if not hasattr(st.session_state, 'allocation'):
        st.markdown("Classical Weight Optimization")

        with st.spinner("Optimizing portfolio weights..."):
            allocation, metrics = classical_weight_optimization(
        ...
        if backtest_results is None:
            ...
            #
            # with col3:
            #     st.metric("CAGR", f"{backtest_results['cagr']:.2%}")
            #
            # with col4:
            #     st.metric("Sharpe", f"{backtest_results['sharpe']:.2f}")
